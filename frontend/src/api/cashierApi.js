...
async function apiRequest(endpoint, options = {}) {
  ...
  try {
    ...
    console.warn(`[Cashier API] Network request failed for ${url}:`, error.message);
    throw error;
  }
}
...
export async function fetchCurrentRegisterSession() {
  try {
    const res = await apiRequest('/cashier/register/current', { method: 'GET' });
    return res.data;
  } catch {
...
export async function fetchRegisterHistory() {
  try {
    const res = await apiRequest('/cashier/register/history', { method: 'GET' });
    return res.data || [];
  } catch {
...
export async function fetchCashierProducts(search = '', barcode = '') {
  try {
    ...
    const res = await apiRequest(`/cashier/products${queryString}`, { method: 'GET' });
    return res.data || [];
  } catch {
...
export async function fetchRecentInvoices(limit = 20) {
  try {
    const res = await apiRequest(`/cashier/sales/recent?limit=${limit}`, { method: 'GET' });
    return res.data || [];
  } catch {
...
export async function resumeHeldBill(holdId) {
  try {
    const res = await apiRequest(`/cashier/held-bills/${holdId}`, { method: 'DELETE' });
    return res.data;
  } catch {
    return null;
  }
...
export async function searchReturnInvoice(invoiceNo) {
  try {
    const res = await apiRequest(`/cashier/returns/search?invoiceNo=${encodeURIComponent(invoiceNo)}`, {
      method: 'GET',
    });
    return res.data;
  } catch {
