...
class CashierService {
  ...
  async openSession({
    ...
    const { organisationId, branchId, cashierId } = await this._resolveContext(
      reqOrgId,
      reqBranchId,
    );
  ...
  async closeSession({ countedCash = 0, notes, denominations }) {
    ...
    try {
      ...
      const expectedCash =
        (cashIn > 0 ? cashIn : openingFloat) + cashSales - cashOut;
    ...
    try {
      ...
      `,
        [
          ...
          type,
          amt,
          reason || (type === "IN" ? "Cash float addition" : "General expense"),
        ],
      );
  ...
  async searchProducts({ search = "", barcode = "", branchId = null }) {
    ...
    const products = res.rows.map((r) => ({
      ...
      batchId: r.batchId,
      expiry: r.expiry
        ? new Date(r.expiry).toISOString().split("T")[0]
        : "2027-12-31",
  ...
  async createSale(saleData) {
    ...
    try {
      ...
      const getItemPrice = (it) =>
        Number(it.price ?? it.sellingPrice ?? it.unitPrice ?? it.mrp ?? 0);
      const getItemQty = (it) => parseInt(it.quantity ?? it.qty ?? 1, 10);

      const computedItemsSubtotal = items.reduce(
        (acc, it) => acc + getItemPrice(it) * getItemQty(it),
        0,
      );
      const subtotal =
        Number(rawSubtotal) > 0
          ? Number(rawSubtotal)
      ...
      for (const it of items) {
        ...
        const lineTotal =
          Number(it.lineTotal || it.totalPrice || it.total || 0) || price * qty;
  ...
  async processReturn(returnData) {
    ...
    try {
      ...
      const totalRefund = items.reduce(
        (acc, it) =>
          acc +
          Number(it.refundPrice || it.price || 0) *
            Number(it.returnQty || it.quantity || 1),
        0,
      );
      ...
      `,
        [
          organisationId,
          branchId,
          origInvoice.customer_id || customerId,
          origInvoice.id,
          cashierId,
          returnNumber,
          totalRefund,
      ...
      for (const it of items) {
        ...
        `,
          [
            returnRecord.id,
            it.invoiceItemId || defaultInvItemId,
            qty,
            refundAmt,
            restock ? qty : 0,
          ],
      ...
      return {
        success: true,
        message:
          "Return processed and refund recorded successfully in PostgreSQL",
        data: {
          id: returnRecord.id,
